...
WAV2LIP_API_URL = os.getenv("WAV2LIP_API", "http://super-wav2lip-optimized:8002") 
...
def check_api_health(retries: int = 3, retry_delay: float = 1.0) -> Dict[str, bool]:
    ...
    # Wav2Lip API チェック（リトライ付き）
    for attempt in range(retries):
        try:
            response = requests.get(f"{WAV2LIP_API_URL}/health", timeout=3)
            if response.status_code == 200:
...
def generate_lipsync_video_with_realtime_progress(
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            # 進捗監視開始（セッションIDがある場合）
            if session_id and progress_callback:
                import threading
                    ...
                    while not stop_polling.is_set():
                        try:
                            progress_response = requests.get(
                                f"{WAV2LIP_API_URL}/progress/{session_id}",
                                timeout=5
                            )
                            if progress_response.status_code == 200:
                                progress_data = progress_response.json()
                                progress = progress_data.get("progress", 0)
                                message = progress_data.get("message", "処理中...")
                                completed = progress_data.get("completed", False)
                                
                                # コールバック呼び出し（75-100%の範囲にマッピング）
                                mapped_progress = 75 + (progress * 0.25)  # 0-100% → 75-100%
                                progress_callback(mapped_progress, message)
                                
                                if completed:
                                    break
                            else:
                                logger.warning(f"⚠️ 進捗取得エラー: {progress_response.status_code}")
                        except Exception as e:
                            logger.warning(f"⚠️ 進捗ポーリングエラー: {e}")
                        
                        time_module.sleep(0.5)  # 500ms間隔でポーリング
                ...
                # メイン処理完了まで待機
                time_module.sleep(1)  # レスポンス取得後少し待つ
                stop_polling.set()  # ポーリング停止
