            ...
            with torch.cuda.amp.autocast():
                with torch.no_grad():  # 勾配計算無効化
                    if len(batch_frames) == 1:
                        # シングル処理（最適化）
                        enhanced_frame = enhance_with_codeformer(batch_frames[0], run_params)
                        if enhanced_frame is None:
                            enhanced_frame = batch_frames[0]
                        enhanced_batch.append(enhanced_frame)
                    else:
                        # 確実なCodeFormer順次処理（最安定）
                        print("バッチ処理内で順次CodeFormer処理を実行💢")
                        for frame in batch_frames:
                            enhanced_frame = enhance_with_codeformer(frame, run_params)
                            if enhanced_frame is None:
                                enhanced_frame = frame
                            enhanced_batch.append(enhanced_frame)
