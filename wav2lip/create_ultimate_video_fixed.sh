#!/bin/bash
# 🎭 ツンデレWav2Lip + CodeFormer 究極統合スクリプト（修正版）
...
docker run --gpus all --rm --privileged \
  ...
  --outfile /app/output/temp_wav2lip_result.mp4 \
  --out_height 720 \
  --quality Fast"

# 出力ファイルを作業ディレクトリにコピー
if [ -f "output/temp_wav2lip_result.mp4" ]; then
    cp "output/temp_wav2lip_result.mp4" "$WAV2LIP_OUTPUT"
    rm "output/temp_wav2lip_result.mp4"
    echo "✅ Wav2Lip処理完了よ！"
else
    echo "❌ Wav2Lip処理失敗...もう！💢"
    exit 1
...
for frame in "../$FRAMES_DIR"/frame_*.png; do
    ...
    # CodeFormer実行
    timeout 10 docker compose exec -T codeformer python /app/codeformer_face_fix.py \
